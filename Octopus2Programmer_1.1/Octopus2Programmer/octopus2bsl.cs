    ...
    public class Octopus2bsl
    {
        ...
        private bool programTIText(string filename, byte action)
        {
            ...
            for (; ; )
            {
                ...
                if (!flag)
                    form.SetProgress( (int)((double)prog_line*100/total_line));
                else
                    form.SetProgress((int)((double)prog_line * 100 / total_line) + 50);
            }
        ...
        public int program_start(string source_ihex_file,string port)
        {
            ...
            int error = 0;
            if (error == 0 && !File.Exists(source_ihex_file))
            {
                form.AppendLog("檔案不存在!\r\n") ;
                error = -1;
            }
            if (error == 0 && !hexToTxt(source_ihex_file, "_tmp_" + port + "_it.txt"))
            {
                form.AppendLog("檔案開啟錯誤!\r\n");
                error = -2;
            }

            if (error == 0 && !comInit(port))
            {
                form.AppendLog("連接埠開啟錯誤!\r\n");
                error = -3;
            }
            ...
            if (error == 0 && !bslTxRx(BSL_MERAS, 0xff00, 0xa506, null, Blkin))
            {
                form.AppendLog("bslTxRx(BSL_MERAS,...)錯誤!\r\n");
                error = -4;
            }
            
            if (error==0 && !txPasswd())
            {
                form.AppendLog("txPasswd()錯誤!\r\n");
                error = -5;
            }

            if (error == 0 && bslTxRx(BSL_RXBLK, 0x0ff0, 14, null, Blkin))
            {
                if (error == 0 && !bslTxRx(BSL_SPEED, (0x87 << 8) | 0xE0, 2, null, Blkin))
                {
                    form.AppendLog("bslTxRx(BSL_SPEED,...)錯誤!\r\n");
                    error = -7;
                }
                ...
                if (error == 0 && !programTIText("_tmp_" + port + "_it.txt", ACTION_ERASE_CHECK))
                {
                    form.AppendLog("programTIText ACTION_ERASE_CHECK 錯誤!\r\n");
                    error = -8;
                }
                
                flag = true;
                if (error == 0 && !programTIText("_tmp_" + port + "_it.txt", ACTION_PROGRAM))
                {
                    form.AppendLog("programTIText ACTION_PROGRAM 錯誤!\r\n");
                    error = -9;
                }
                duration_time = (double)(Environment.TickCount - start_time) / 1000;
                bslReset(false);
            }
            else if(error==0)
            {
                form.AppendLog("bslTxRx(BSL_RXBLK,...)錯誤!\r\n");
                error = -6;
            }
            
            if(error<=-3 || error==0)
                comport.Close();
            File.Delete("_tmp_" + port + "_it.txt");
